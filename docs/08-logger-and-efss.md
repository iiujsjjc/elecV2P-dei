...
\`\`\` JS nodejs const { logger } = require\('./utils'\) const clog = new logger\({ head: 'example', level: 'debug', file: 'test' }\) clog.info\('hello elecV2P!'\)
...
例如：[http://127.0.0.1/efss](http://127.0.0.1/efss)
...
**./** - 相对目录。相对当前工作目录 例如：./script/Shell, ./logs

**/** - 绝对目录。 例如： /etc, /usr/local/html

_如果目录中包含大量静态文件，例如直接设置为根目录 **/**，可能引用比较耗时，请合理设置 efss 目录_
